...
# 创建一个直接的修复脚本
cat > ./fix-nitro-direct-v2.js << EOL
...
const fixFile = async () => {
  ...
  try {
    ...
    content = content.replace(
      /const\s+require\s*=\s*createRequire/g, 
...
const PrismaClient = (() => {
  try {
    ...
    return class MockPrismaClient {
      constructor() {
        console.log('使用模拟的 PrismaClient');
      }
      async $connect() { return this; }
      async $disconnect() { return this; }
    };
  } catch (error) {
    console.error('创建模拟 PrismaClient 失败:', error);
    return class {};
  }
})();\`
    );
