...
async function processArticle(article: any): Promise<void> {
  try {
    logger.info(`开始处理金十数据文章: ${article.title}`);

    // 1. 获取文章详细内容
    const articleDetail = await fetchJin10ArticleDetail(article.id);
    if (!articleDetail) {
      logger.error(`获取金十数据文章详情失败: ${article.title}`);
      return;
    }
    ...
    logger.info(`金十数据文章处理完成: ${article.title}`);
  } catch (error) {
    logger.error(`处理金十数据文章失败: ${article.title}`, { error: error instanceof Error ? error.message : String(error) });
  }
}
...
async function executeTask(): Promise<void> {
  try {
    ...
    if (articles.length === 0) {
      logger.warn('未获取到金十数据文章，跳过本次处理');
      return;
    }
    
    logger.info(`获取到 ${articles.length} 篇金十数据文章，将处理前 ${BATCH_SIZE} 篇`);
    ...
    if (newArticles.length === 0) {
      logger.info('所有金十数据文章已存在于数据库中，跳过本次处理');
      return;
    }
    
    logger.info(`发现 ${newArticles.length} 篇新金十数据文章需要处理`);
    ...
    logger.info(`本次金十数据任务处理完成，共处理 ${newArticles.length} 篇文章`);
  } catch (error) {
    logger.error('执行金十数据文章处理任务失败', { error: error.message });
  }
}
