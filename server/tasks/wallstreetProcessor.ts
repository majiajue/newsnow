...
async function processArticle(article: any): Promise<void> {
  try {
    console.log(`开始处理华尔街见闻文章: ${article.title}`);

    // 1. 直接使用爬虫获取文章详细内容，不使用JinaAI
    console.log(`使用爬虫获取华尔街见闻文章详情: ${article.id}`);
    const articleDetail = await fetchWallStreetArticleDetail(article.id);
    if (!articleDetail) {
      console.error(`获取华尔街见闻文章详情失败: ${article.title}`);
      return;
    }
    ...
    const content = {
      ...
      author: articleDetail.author || '华尔街见闻',
    ...
    console.log(`华尔街见闻文章处理完成: ${article.title}`);
  } catch (error) {
    console.error(`处理华尔街见闻文章失败: ${article.title}`, { error: error instanceof Error ? error.message : String(error) });
  }
}
...
async function executeTask(): Promise<void> {
  try {
    ...
    if (articles.length === 0) {
      console.warn('未获取到华尔街见闻文章，跳过本次处理');
      return;
    }
    
    console.log(`获取到 ${articles.length} 篇华尔街见闻文章，将处理前 ${BATCH_SIZE} 篇`);
    ...
    if (newArticles.length === 0) {
      console.log('所有华尔街见闻文章已存在于数据库中，跳过本次处理');
      return;
    }
    
    console.log(`发现 ${newArticles.length} 篇新华尔街见闻文章需要处理`);
    ...
    console.log(`本次华尔街见闻任务处理完成，共处理 ${newArticles.length} 篇文章`);
  } catch (error) {
    console.error('执行华尔街见闻文章处理任务失败', { error: error.message });
  }
}
