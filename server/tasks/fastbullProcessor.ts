/**
 * FastBull文章处理定时任务
 * 自动获取FastBull文章，使用JinaAI提取内容，用DeepSeek生成摘要和评论，并存储到数据库
 */
...
// 任务ID
const TASK_ID = 'fastbull-processor';
// 任务执行间隔（毫秒）
...
async function fetchArticleContent(url: string): Promise<any> {
  try {
    console.log(`开始从Jina AI获取文章内容: ${url}`);
...
async function saveArticleToDatabase(article: any, content: any, ai: any): Promise<void> {
  try {
    ...
    // 确保content字段不为空
    const articleContent = content.content || article.content || '无法获取文章内容';
    if (!articleContent || articleContent.trim().length === 0) {
      console.error(`文章内容为空，无法保存: ${article.title}`);
      return;
    }
    ...
    try {
      ...
      if (isNaN(publishDate.getTime())) {
        ...
        console.warn(`文章发布日期无效，使用当前时间: ${article.title}`);
      }
    } catch (e) {
      publishDate = new Date();
      console.warn(`处理文章发布日期出错，使用当前时间: ${article.title}`);
    }
    ...
    const dataToCreate = {
      ...
      sourceUrl: article.url,
      author: content.author || article.author || 'FastBull',
      publishDate: publishDate,
      categories: article.category || '财经',
      summary: ai.summary || article.title,
    ...
    console.log(`准备保存到数据库的文章数据:`, JSON.stringify(dataToCreate, null, 2));
    ...
    console.error(`保存文章到数据库失败: ${article.title}`, { error: errorMessage, stack: errorStack });
    throw error;
  }
}
...
async function processArticle(article: any): Promise<void> {
  try {
    console.log(`开始处理FastBull文章: ${article.title}`);

    // 1. 直接使用爬虫获取文章详细内容，不再使用Jina AI
    console.log(`使用爬虫获取FastBull文章详情: ${article.id}`);
    const articleDetail = await fetchFastBullArticleDetail(article.id);
    if (!articleDetail) {
      console.error(`获取FastBull文章详情失败: ${article.title}`);
      return;
    }
    
    // 2. 提取和清理爬虫获取的内容
    const rawCrawlerContent = articleDetail.content || '';
    const cleanContent = rawCrawlerContent.replace(/<[^>]*>/g, '').trim();
    ...
    console.log(`成功获取文章内容，原始内容长度: ${rawCrawlerContent.length}，清理后长度: ${cleanContent.length}`);
    
    // 3. 封装文章内容
    const content = {
      title: articleDetail.title || article.title,
      content: rawCrawlerContent,
    ...
    if (contentToUse.length < 50) {
      contentToUse = article.title + (article.summary ? '. ' + article.summary : '');
      console.warn(`文章内容过短，使用标题和描述作为备选内容: ${contentToUse}`);
    }
    ...
    console.log(`FastBull文章处理完成: ${article.title}`);
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    const errorStack = error instanceof Error ? error.stack : 'No stack trace';
    console.error(`处理FastBull文章失败: ${article.title}`, { error: errorMessage, stack: errorStack });
  }
}
...
async function executeTask(): Promise<void> {
  try {
    // 1. 获取最新文章列表
    const articles = await fetchFastBullNews(1, 20);
    
    if (articles.length === 0) {
      console.warn('未获取到FastBull文章，跳过本次处理');
      return;
    }
    
    console.log(`获取到 ${articles.length} 篇FastBull文章，将处理前 ${BATCH_SIZE} 篇`);
    ...
    if (newArticles.length === 0) {
      console.log('所有FastBull文章已存在于数据库中，跳过本次处理');
      return;
    }
    
    console.log(`发现 ${newArticles.length} 篇新FastBull文章需要处理`);
    ...
    console.log(`本次FastBull任务处理完成，共处理 ${newArticles.length} 篇文章`);
  } catch (error) {
    console.error('执行FastBull文章处理任务失败', { error: error.message });
  }
}
 ...
 * 注册FastBull文章处理任务
 */
export function registerFastBullProcessorTask(): void {
  registerTask({
    id: TASK_ID,
    name: 'FastBull文章处理器',
  ...
  logger.info(`FastBull文章处理任务已注册，间隔: ${TASK_INTERVAL / 1000 / 60}分钟`);
}
